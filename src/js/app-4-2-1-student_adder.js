  ...
  ShowAllStudents: function () {
    ...
    App.contracts.Professor.deployed()
      .then(function (instance) {
        ...
        // 先获得所有的地址
        return instance_.getCourseInfByCourseId(show_course_id, {
          from: account,
          gas: 300000,
        });
      })
      .then(function (courseInf_) {
        ...
        if (courseInf_[0] == 0) {
          ...
          var proAddress =
            courseInf_[3].slice(0, 6) +
            ".." +
            courseInf_[3].slice(proAddressLength - 4, proAddressLength);
          // let proName = await instance_.getProfessorNameByAddress(courseInf_[2], {from: account, gas: 300000});

          // 展示课程基本信息
          var courInfHead_ =
            "<caption><h2>Course Information</h2></caption" +
            "<thead><tr><th>ID</th>" +
            "<th>Name</th>" +
      ...
      .then(function (allStudentAddress_) {
        ...
        if (sum == 0) {
          ...
          for (var i = 0; i < sum; i++) {
            instance_
              .getStudentInfByStuAddress(allStudentAddress_[i], {
                from: account,
                gas: 300000,
              })
  ...
  ApplyCourse: function (stuId) {
    ...
    App.contracts.Professor.deployed()
      ...
      .then(function (apply_student_address) {
        ...
        return instance_.applyCourse(
          apply_course_id,
          account,
          apply_student_address,
  ...
  ShowAddressInf: function () {
    ...
    App.contracts.Professor.deployed()
      .then(function (instance) {
        ...
        nowID = instance.getIdByAddress(account, {
          from: account,
          gas: 300000,
        });
        return nowID;
    ...
    App.contracts.Professor.deployed()
      .then(function (instance) {
        ...
        nowAuthorization = instance.getAuthorizationByAddress(account, {
          from: account,
          gas: 300000,
        });
      ...
      .then(function (nowAuthorization) {
        if (nowAuthorization != 2) {
          alert(
            "Only Professors Have Access To This Page.\n Redirecting to Main Page."
          );
